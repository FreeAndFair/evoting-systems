    ...
    <xsl:for-each select="//row">
      ...
      <tr>
	   ...
	   <xsl:if test="node()= 0">
	      ...
	      <xsl:attribute name="src">/iTalyVotingDemo/xml/images/Row-Box-Print.jpg</xsl:attribute>
             </xsl:element>
 	   </xsl:if>	 
           <xsl:if test="node()= 1">
 	     <xsl:element name="img">
	      <xsl:attribute name="src">/iTalyVotingDemo/xml/images/Row-<xsl:value-of select="$row-no"/>-Item-<xsl:value-of select="attribute::node()" />-selected.jpg</xsl:attribute>
             </xsl:element>
	   </xsl:if>	 
	 </td>
         </xsl:if>
