...
void get_dummy_rotation(const struct electorate *elec);
...
void get_rotation(const struct electorate *elec)
{
	...
	reply = http_exchange(TEST_SERVER_ADDRESS, TEST_SERVER_PORT, ROBSON_CGI,request);
	if (!reply)
...
void get_dummy_rotation(const struct electorate *elec)
{
	unsigned int i;
	for (i = 0; i < elec->num_seats; i++) {
		current_rotation.rotations[i] = i;
	}
	/* DDS3.2.6: Save Rotation */
	current_rotation.size = elec->num_seats;
}
