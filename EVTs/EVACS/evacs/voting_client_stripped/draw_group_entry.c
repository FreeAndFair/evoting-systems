...
/* SIPL 2011-06-29 Take the possibility of split groups into account. */
					   ...
					   int candidate_index)
{
	...
	/* SIPL 2011-06-29 Calculate the physical column and grid block
	   based on group_index and the ballot data. */
	unsigned int physical_column;
	unsigned int grid_block;

	/* SIPL 2011-06-29 Need the ballot to work out where
	...
	*/
	...
	/* SIPL 2011-06-29 Now use grid_block instead of group_index. */
...
//fprintf(stderr, "Initial x=%d, y=%d\n", coordinates.x, coordinates.y); 
//fprintf(stderr, "preference_image_size=%d\n", get_preference_image_size(voter_electorate)); 
	/* Now calculate coordinates of this candidate */
	/* SIPL 2011-06-29 . . . or group. */
...
//fprintf(stderr, "calculating coords for grp=%d, cand=%d\n", cursor.group_index, cursor.screen_candidate_index); 
...
/* SIPL 2011-06-29 Do the same as calc_curs_coord, but take
