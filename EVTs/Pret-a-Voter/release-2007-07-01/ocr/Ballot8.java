...
public class Ballot8 
{ 
  public static void main (String args[]) throws InterruptedException
  {
    if (args.length < 1) 
    {
        System.err.println ( "Usage: java Ballot8 image_file_name");
        System.exit(1);
    }
    ...
    String [] raceResult = new String [] {"","","","","","",""};
    String [] raceMessage = new String [] {"","","","","","",""};
...
// }
    ...
    for (int hscan = topleft.x + (topright.x - topleft.x)*5/1000; 
             hscan < (topleft.x + (topright.x - topleft.x)*995/1000);
             hscan++)
    { int blackness = 0;
      int trans = 0;
      int t2 = 0;
    ...
    if (((topright.x - topleft.x) < 700) || 
        ((topright.x - topleft.x) > 900) ||
        ((bottomleft.y - topleft.y) < 3100) ||
        ((bottomleft.y - topleft.y) > 3400) )
    {
      System.err.println ("99,0,00000000000,0,0,0,0,0,0,0,Central Section is wrong size," + args[0] + ",,,,,,");
      System.out.println ("99,0,00000000000,0,0,0,0,0,0,0,Central Section is wrong size," + args[0] + ",,,,,,");
      System.exit(1);
    }
    ...
    { 
      ...
      for (int cell=0; cell<numCands; cell++)
      {
        ...
        if ((cellInnerWidth < 110) ||
            ...
            (cellInnerHeight > 140) )
        { 
          System.err.println ("98,0,00000000000,0,0,0,0,0,0,0,Cell is wrong size," + args[0] + ",,,,,,");
          System.out.println ("98,0,00000000000,0,0,0,0,0,0,0,Cell is wrong size," + args[0] + ",,,,,,");
...
*/
    ...
    int cnt4 = floodfill (myimage,
                         ...
                         new Pair ((topleft.x+bottomleft.x)/2 +
                                 ((bottomright.x+topright.x)-
                                  ...
                                  125),   //FIXME  points[0].y),
                         new Pair ((topleft.x+bottomleft.x)/2 +
                                 ((bottomright.x+topright.x)-
                                  (topleft.x+bottomleft.x))/2*baroff[1], 
                                  1200),    // FIXME points[1].y),
                         null,   /* new int[] {0,0,255,0}, */
    ...
    double hbit = ((bcodetopright.x-bcodetopleft.x) / 25.049);
    double vbit = ((bcodebottomleft.y-bcodetopleft.y) / 65.04);
    ...
    if (debug)
    {
      System.err.println ("Boundary count = " + cnt4);
    ...
    { double vpos = bcodetopleft.y + 
            ((double)(bcodebottomleft.y-bcodetopleft.y))/(bcodeHeight+0.8) * 
                                                                 (0.9 + brow);
      int [] rowbits = new int [bcodeWidth];
      for (int bcol=0; bcol<bcodeWidth; bcol++)
      { double hpos = bcodetopleft.x +
            ((double)(bcodetopright.x-bcodetopleft.x))/(bcodeWidth+0.9) * 
                                                                (0.9 +bcol);
