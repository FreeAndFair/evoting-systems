...
$fn_ctbs = shift @ARGV;
$fn_re = shift @ARGV;
die "Usage: audit_tabulate.pl BLANK_BALLOT_FILE CLEAR_TEXT_BALLOT_FILE RESULTS_FILE " unless defined $fn_bb && $fn_ctbs && $fn_re;
...
$results_tree = XML_Tree->construct ("Results");
foreach (sort keys %$questions) {
   my $q = $_;
   my $ques = $results_tree->new_element ("Question");
...
# Update results tree for each voted ballot
my $ctbs_tree = XML_Tree->new (snarf ($fn_ctbs));
foreach my $ctb_tree (grep {$_->name () eq "ClearTextBallot"} @{$ctbs_tree->elements ()}) {
   foreach my $ans_tree (grep {$_->name () eq "AnswerReference"} @{$ctb_tree->elements ()}) {
      my $a = $ans_tree->characters ();
      foreach (sort keys %$questions) {
         my $q = $_;
         foreach my $re_q (grep {$_->a ("QuestionReference") eq $q} @{$results_tree->elements ()}) {
            foreach my $re_a (grep {$_->a ("AnswerReference") eq $a} @{$re_q->elements ()}) {
...
unsnarf ($results_tree->internal_format_as_text () . "\n", "$fn_re");
