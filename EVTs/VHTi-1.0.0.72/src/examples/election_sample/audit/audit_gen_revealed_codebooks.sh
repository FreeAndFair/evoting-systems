...
# Usage
if test $# -lt 4 ; then
   echo "Usage: audit_gen_revealed_codebooks.sh NUMBER_TRUSTEES CODE_BIT_LENGTH ELECTION_ID PRECINCT_ID"
   exit 2
fi
n=$1
vcbitlen=$2
...
echo "AUDIT: checking codebook revealed secrets ... "
tid=1
...
echo -n "AUDIT: generating revealed codebooks ... "
vhti_call combine_dictionary_secrets \
   ../transcript/precinct_$eid-$pid/tabulation/trustee_revealed_codebook_secrets_box.xml \
   ../transcript/precinct_$eid-$pid/config/blank_ballot.xml \
   $vcbitlen \
   $vcencoding \
   ./precinct_$eid-$pid/trustee_revealed_codebooks.xml
echo "done."

# Comparing revealed codebooks
echo -n "AUDIT: comparing AUDIT and LEO revealed codebook files ... "
declare -a _result=$(diff -q \
