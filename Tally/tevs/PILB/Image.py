...
class Image:
    ...
    ##
    # Return a copy of this image with dark regions thickened
    #
    # @return An Image object.

    def thicken(self):
        "Return a thickened copy"

        self.load()
        im = self.im.thicken(self.im.size[0],self.im.size[1])
        return self._new(im)
    ...
    ##
    # Returns list of horiz and then vert dark lines > 10% of span.
    #
    # @param xy x is the threshold between light and dark, y is allowed misses.
    # @return List of lines, each line (x1,y1,x2,y2)

    def getlines(self, threshold, allowed_misses):
        "Get significant vertical, horizontal dark lines in image"

        self.load()
        return self.im.getlines(threshold, allowed_misses)

    ##
    # Returns list of horiz and then vert light lines > 2/3 of span.
    #
    # @param xy x is the threshold between light and dark, y is allowed misses.
    # @return Merged list of lines or rectangles, each as (x1,y1,x2,y2)

    def getgaps(self, xy):
        "Get significant vertical, horizontal white gaps in image"

        self.load()
        return self.im.getgaps(xy)

    ##
    # Returns list of horiz light lines > 95% of span.
    #
    # @param xy x is the threshold between light and dark, y is allowed misses.
    # @return Merged list of lines or rectangles, each as (x1,y1,x2,y2)

    def gethgaps(self, xy):
        "Get significant vertical, horizontal white gaps in image"

        self.load()
        return self.im.gethgaps(xy)

    ##
    # Returns y offset into image of first "darkish" region.
    #
    # @param x is the threshold between light and dark
    # @return y offset as integer

    def getfirstdark(self, xy):
        "Get y offset of first dark spot in image"

        self.load()
        return self.im.getfirstdark(xy)
    ...
    def getdarkextents(self, x1, y1, x2, y2):
        "Get min and max y value with 2 or more dark pixels"

        self.load()
        return self.im.getdarkextents(x1, y1, x2, y2)

    ##
    # Returns y offsets of substantial intensity changes at provided x offset
    #
    # @param x is the x offset at which to check pixel values
    # @return y offsets as integer list

    def getchanges(self, xtop, xbottom, dpi):
        "Get y offset of first dark spot in image"

        self.load()
        return self.im.getchanges(xtop,xbottom,dpi)

    ##
    # Returns 1 if ESS, 2 if Diebold, 3 if Hart, 0 if cannot determine
    #
    # @param dpi is the dots per inch of the image
    # @return y offset as integer

    def getballotbrand(self, dpi):
        "Get y offset of first dark spot in image"

        self.load()
        return self.im.getballotbrand(dpi)

    ##
    # Returns list of large glyphs
    #
    # @param minw is minimum width in pixels
    # @param minh is minimum height in pixels
    # @return list of matches

    def getbigglyphs(self, minw, minh):
        "Get y offset of first dark spot in image"

        self.load()
        return self.im.getbigglyphs(minw,minh)
    ...
    def getesstilt(self, dpi):
        "Get y offset of first dark spot in image"

        self.load()
        return self.im.getesstilt(dpi)

    ##
    # Returns further analysis
    #
    # @param dpi is the dots per inch of the image
    # @return list of: 

    def getessheaderscodesovals(self, dpi,blocktype, blockx, blocky, linediff, ydiff):
        "Get y offset of first dark spot in image"

        self.load()
        return self.im.getessheaderscodesovals(dpi, blocktype, blockx, blocky, linediff, ydiff)
    ...
    def getessheadersandcodes(self, dpi,blocktype, blockx, blocky, linediff, ydiff):
        "Get y offset of first dark spot in image"

        self.load()
        return self.im.getessheadersandcodes(dpi, blocktype, blockx, blocky, linediff, ydiff)

    ##
    # getesslandmarks has been split into getesstilt and getessheaderscodesovals
    # Returns ess landmarks list
    #
    # @param dpi is the dots per inch of the image
    # @param need_ovals indicates whether we are also asking for vote ops
    # @return y offset as integer

    #def getesslandmarks(self, dpi, need_ovals):
    #    "Get y offset of first dark spot in image"
    #
    #    self.load()
    ...
    def gethartlandmarks(self, dpi, need_vops):
        "Get list of four box corner coord pairs"

        self.load()
        return self.im.gethartlandmarks(dpi, need_vops)
    ...
    def getdieboldlandmarks(self, dpi, need_vops):
        "Get list of four coord pairs at inner x, outer y of extreme dashes"

        self.load()
        return self.im.getdieboldlandmarks(dpi, need_vops)

    ##
    # Returns x offset into image of first vert repeating LdLd region.
    #
    # @param x is the threshold between light and dark
    # @return x offset as integer

    def hasvdashes(self, xy):
        "Return first x at which vertical dash pattern detected, or -1"

        self.load()
        return self.im.hasvdashes(xy)

    ##
    # Returns list of dark/light transitions in zone between x1,x2 starting y1
    #
    # @param thresh is the threshold between light and dark
    # @param x1 is the starting column offset
    # @param y1 is the starting row offset
    # @param x2 is the ending column offset
    # @return x offset as integer

    def getvdashes(self, thresh,x1, y1, x2):
        "Return first x at which vertical dash pattern detected, or -1"

        self.load()
        return self.im.getvdashes(thresh,x1,y1,x2)

    ##
    # Returns list of dark/light transitions in zone between x1,x2 starting y1
    #
    # @param thresh is the threshold between light and dark
    # @param startx is the starting column offset
    # @param starty is the starting row offset
    # @param endx is the ending column offset
    # @return x offset as integer

    def getcolumnvlines(self, startx, starty, endx):
        "Return first x at which vertical dash pattern detected, or -1"

        self.load()
        return self.im.getcolumnvlines(startx,starty,endx)

    ##
    # Given an x offset, return the list of potential hlines sprouting
    #
    # @param startx is the starting column offset
    # @param starty is the starting row offset
    # @param dpi is the dots per inch of the image
    ...
    def getpotentialhlines(self, startx, starty, dpi):
        "Return list of potential horizontal lines sprouting from the vline"

        self.load()
        return self.im.getpotentialhlines(startx,starty,dpi)

    ##
    # Given an x offset, return the list of Hart vote boxes 1/8" from that offset
    #
    # @param startx is the starting column offset
    # @param starty is the starting row offset
    # @param dpi is the dots per inch of the image
    ...
    def gethartvoteboxes(self, startx, starty, dpi):
        "Return list of potential vote boxes just right of the vline"

        self.load()
        return self.im.gethartvoteboxes(startx,starty,dpi)

    ##
    # Given a bounding box and an oval width and height in pixels
    # return a list of the pixel coordinates of voteovals
    # within the bounding box
    #
    # @param startx is the starting column offset
    # @param starty is the starting row offset
    # @param w is the width in pixels of the bounding box
    # @param h is the height in pixels of the bounding box
    # @param ow is the width in pixels of a horizontal oval line
    # @param oh is the distance in pixels between upper and lower
    # horizontal oval line
    # @return list of (x,y) coords of ovals

    def getdieboldvoteovals(self, startx, starty, width, height, ow, oh):
        "Return list of potential vote ovals"

        self.load()
        return self.im.getdieboldvoteovals(startx,starty,width,height,ow,oh)
    ...
    # @param y1 is the starting row offset
    # @param x2 is the ending column offset
    # @param y2 is the ending row offset
    # @return y values as integer list

    def gethdashes(self, thresh,x1, y1, x2, y2):
        "Return first x at which vertical dash pattern detected, or -1"

        self.load()
        return self.im.gethdashes(thresh, x1, y1, x2, y2)
    ...
    # @param starty is the starting y value for the search
    # @return bitcode as integer

    def diebolddashcode(self, thresh, dpi, starty):
        "Return bitcode with 1 for each position with dash, 0 for w/o"

        self.load()
        return self.im.diebolddashcode(thresh, dpi, starty)

    ##
    # Returns list of dark/light transitions in zone between y1,y2 from x1,x2
    #
    # @param x is the threshold between light and dark
    # @return x values as integer list

    def hashdashes(self, xy):
        "Return first y at which horizontal dash pattern detected, or -1"

        self.load()
        return self.im.hashdashes(xy)

    ##
    # Returns avg intensity of red, and four bin histogram of pixels.
    #
    # @param x is the starting x offset of the tested region
    # @param y is the starting y offset of the tested region
    # @param w is the width of the tested region
    # @param h is the height of the tested region
    # @return avg r,g,b then lowest r,g,b, low, high, highest (all r,g,b)

    def cropstats(self, dpi , gap , x , y , w , h , adj):
        "Return list of statistics connected with specified region of image"

        self.load()
        return self.im.cropstats(dpi , gap , x , y , w , h , adj)
    ...
    # @param w is the width of the tested region
    # @param h is the height of the tested region
    # @param tfa is the argument passed to the testfunc used on each pixel
    # @return y offset as integer

    def getblobs(self, x,y,w,h,tfa):
        "Return list of statistics connected with specified region of image"

        self.load()
        return self.im.getblobs(x,y,w,h,tfa)
    ...
    # @param w is the width of the tested region
    # @param h is the height of the tested region
    # @param minw is the min width of a returnable blob
    # @param minh is the height of a returnable blob
    # @param tfa is the argument passed to the testfunc used on each pixel
    # @return y offset as integer

    def getwideblobs(self, x,y,w,h,minw,minh,tfa):
        "Return list of statistics connected with specified region of image"

        self.load()
        return self.im.getwideblobs(x,y,w,h,minw,minh,tfa)

    ##
    # Return list of tinted blobs
    #
    # @param x is the starting x offset of the tested region
    # @param y is the starting y offset of the tested region
    # @param w is the width of the tested region
    ...
    def get_tinted_blobs(self, x,y,w,h,tfa):
        "Return list of statistics connected with specified region of image"

        self.load()
        return self.im.get_tinted_blobs(x,y,w,h,tfa)

    ##
    # Return list of lengths of white black stretches
    #
    # @param x is the starting x offset of the tested region
    # @param y is the starting y offset of the tested region
    # @param w is the width of the tested region
    ...
    def getbarcode(self, x,y,w,h):
        "Return list of lengths of white/black stretches in pattern."

        self.load()
        return self.im.getbarcode(x,y,w,h)
