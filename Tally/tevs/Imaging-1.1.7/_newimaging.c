...
#define MAXCODE 80
  PyObject *item;
  PyObject *list;
  /*start 1" in and .4" down, looking over 1" for consistent dark */
  point9 = (9*dpi)/10;
  point6 = (6*dpi)/10;
  point5 = dpi/2;
  point4 = (4*dpi)/10;
  ...
  for (y=point3;y<dpi;y++) {
    ...
    //printf("rightred %d\n",rightred);
    if (((leftred/dpi) < 96) && (found_topline_left == 0)){
      found_topline_left = y;
      //printf("found topline left %d\n",found_topline_left);
    }
    if (((rightred/dpi) < 96) && (found_topline_right == 0)){
...
#endif
  /* look for bottom line on left and right, 1" to 2" in from horiz edges, 
     starting 0.4" in from vertical edges */
  for (y = (im->ysize - point3) ; y > (im->ysize - (4*dpi)) ; y--) {
    ...
    if (((leftred/dpi) < 96) && (found_bottomline_left == 0)){
      found_bottomline_left = y;
    }
    if (((rightred/dpi) < 96) && (found_bottomline_right == 0)){
  ...
  for (x=point3;x<dpi;x++) {
    topred = 0;
    bottomred = 0;
    for (y=found_topline_left ; y<(found_topline_left+dpi) ; y++){
      p = (UINT8*) &im->image32[y][x];
      topred += p[0];
    }
    for (y=found_bottomline_left ; y>(found_bottomline_left-dpi) ; y--){
      p = (UINT8*) &im->image32[y][x];
      bottomred += p[0];
    }
    ...
    if (((topred/dpi) < 96) && (found_leftline_top == 0)){
#ifdef VERBOSE
      printf("Found leftline top at %d\n",x);
#endif
      found_leftline_top = x;
    }
    if (((bottomred/dpi) < 96) && (found_leftline_bottom == 0)){
#ifdef VERBOSE
  ...
  for (x=im->xsize - point3 ; x > im->xsize - dpi ; x--) {
    ...
    for (y=found_topline_right ; y<(found_topline_right+dpi) ; y++){
      p = (UINT8*) &im->image32[y][x];
      topred += p[0];
#ifdef VERBOSE
      printf("x=%d, y=%d\n",x,y);
    ...
    for (y=found_bottomline_right ; y>(found_bottomline_right-dpi) ; y--){
      p = (UINT8*) &im->image32[y][x];
      bottomred += p[0];
#ifdef VERBOSE
      printf("x=%d, y=%d\n",x,y);
    ...
    if (((topred/dpi) < 96) && (found_rightline_top == 0)){
      found_rightline_top = x;
    }
    if (((bottomred/dpi) < 96) && (found_rightline_bottom == 0)){
...
static inline PyObject*
cropstats(Imaging im, int x, int y, int w, int h, int fine_adj)
{
  UINT8 *p;
  UINT8 *p1;
  UINT8 *p2;
  ...
  gap_width = dpi/18;
  thinstart_to_thickstart_width = dpi/15;
  ...
  if (fine_adj != 0){
    ...
    found = 0;
    for (pty = y - (dpi/10); pty < y + (dpi/10); pty++){
  ...
  retlist = Py_BuildValue("iiiiiiiiiiiiiiiii", 
...
_getlines(ImagingObject* self, PyObject* args)
{
    ...
    return getlines(self->image, x, y);
}
...
_cropstats(ImagingObject* self, PyObject* args)
{
  int x,y,w,h,adj;
  int ok;
  ok = PyArg_ParseTuple(args,"iiiii",&x,&y,&w,&h,&adj);
  
  if (!ok) return NULL;

  return cropstats(self->image, x,y,w,h,adj);
}
